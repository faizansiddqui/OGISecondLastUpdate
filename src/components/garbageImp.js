...
// const PublicProfile = ({ profileUser }) => {
...
const fetchUsersProfiles = async () => {
  try {
    ...
    const handleChat = (postUserId) => {
        if (user && postUserId) {
            router.push(`/chat/${user.uid}_${postUserId}`);
        } else {
            alert("Unable to initiate chat. Please ensure you are logged in.");
        }
    };
                ...
                {posts.length > 0 ? (
                    posts.map((post) => (
                        ...
                        >
                            ...
                            {/* Display post owner's UID */}
                            {post.user && <p>User UID: {user.uid}</p>}

                            {/* Chat button to start a chat with the post owner */}
                            <div>
                                <h2>All User Profiles:</h2>
                                {userProfile.length > 0 ? (
                                    userProfile.map((profile) => (
                                        <div key={profile.id}>
                                            <h3>{profile.username} (UID: {profile.id})</h3>
                                            <button onClick={() => handleChat(profile.id)}>Chat</button>
                                        </div>
                                    ))
                                ) : (
...
const UserProfile = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = auth.onAuthStateChanged((authUser) => {
      ...
      const { uid } = authUser;
      if (uid) {
        // Dispatch fetchUserPosts with UID
    ...
    return posts.map((post) => (
      <div key={post.id} style={{ border: "1px solid #ccc", margin: "10px", padding: "10px" }}>
        ...
        {post.fileType === "video" ? (
          ...
          <Image src={post.fileUrl} alt={post.title} width={500} height={300} quality={80} priority />
        )}
        <p>Posted on: {new Date(post.createdAt).toLocaleString()}</p>
      </div>
    ));
